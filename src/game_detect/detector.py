...
class ContextDetector:
    ...
    IGNORED_TITLES = ["AIgator", "Настройки"]
    ...
    def get_active_window_context(self) -> Optional[AppContext]:
        """Получить контекст активного окна (игнорируя AIgator)"""
        try:
    ...
    def _identify_app(self, process_name: str, window_title: str) -> tuple[str, str]:
        ...
        if any(
            x in title_lower
            for x in [".py", ".js", ".ts", ".cpp", ".java", ".cs", "visual studio"]
        ):
            return (
                window_title.split(" - ")[-1].strip()
                if " - " in window_title
                else window_title,
                "ide",
            )
